...
const feelingColorMap: Record<string, { hue: number; saturation: number; lightness: number }> = {
...
function generatePalette(baseColor: { hue: number; saturation: number; lightness: number }): string[] {
  ...
  palette.push(hslToHex(hue1, baseColor.saturation * 0.9, baseColor.lightness * 1.1))
  palette.push(hslToHex(baseColor.hue, baseColor.saturation, baseColor.lightness))
  palette.push(hslToHex(hue2, baseColor.saturation * 0.8, baseColor.lightness * 0.9))

  // Add a complementary color (opposite on the color wheel)
  const complementaryHue = (baseColor.hue + 180) % 360
  palette.push(hslToHex(complementaryHue, baseColor.saturation * 0.7, baseColor.lightness * 0.8))
...
function generateMonochromaticPalette(baseColor: { hue: number; saturation: number; lightness: number }): string[] {
  return [
    hslToHex(baseColor.hue, baseColor.saturation * 0.8, baseColor.lightness * 1.3),
    hslToHex(baseColor.hue, baseColor.saturation * 0.9, baseColor.lightness * 1.15),
    hslToHex(baseColor.hue, baseColor.saturation, baseColor.lightness),
    hslToHex(baseColor.hue, baseColor.saturation * 1.1, baseColor.lightness * 0.85),
    hslToHex(baseColor.hue, baseColor.saturation * 1.2, baseColor.lightness * 0.7),
  ]
}

// Generate triadic palette (three colors evenly spaced on the color wheel)
function generateTriadicPalette(baseColor: { hue: number; saturation: number; lightness: number }): string[] {
  const hue1 = (baseColor.hue + 120) % 360
  const hue2 = (baseColor.hue + 240) % 360

  return [
    hslToHex(baseColor.hue, baseColor.saturation, baseColor.lightness),
    hslToHex(baseColor.hue, baseColor.saturation * 0.7, baseColor.lightness * 1.1),
...
function addRandomization(color: { hue: number; saturation: number; lightness: number }): {
  hue: number
  saturation: number
  lightness: number
} {
  return {
    ...
    saturation: Math.min(100, Math.max(0, color.saturation + Math.floor(Math.random() * 20) - 10)),
    lightness: Math.min(100, Math.max(0, color.lightness + Math.floor(Math.random() * 20) - 10)),
  }
}
...
export async function generatePalettes(feeling: string): Promise<string[][]> {
  try {
    ...
    // Convert the response to the expected format (array of color arrays)
    return data.palettes.map((palette) => palette.colors)
  } catch (error) {
